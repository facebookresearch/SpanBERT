...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        if isinstance(vocab_size_or_config_json_file, str):
            ...
            self.clamp_attention = clamp_attention
        else:
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.clamp_attention = config.clamp_attention
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, max_pair_targets, bert_model_embedding_weights, pair_pos_embedding_size, pair_target_layer=-1, no_nsp=False):
        super(BertPreTrainingHeads, self).__init__()
        self.pair_target_layer = pair_target_layer
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
        self.pair_target_predictions = BertPairTargetPredictionHead(config, bert_model_embedding_weights, max_pair_targets)
    ...
    def forward(self, sequence_output, pairs, pooled_output):
        prediction_scores = self.predictions(sequence_output[-1])
        pair_target_scores = self.pair_target_predictions(sequence_output[self.pair_target_layer], pairs)
...
class BertModel(PreTrainedBertModel):
    ...
    ```
    """
    def __init__(self, config, remove_head=False, remove_pooled=False, no_nsp=False):
        ...
        if not remove_head:
            ...
            if not remove_pooled and not no_nsp:
                self.pooler = BertPooler(config)
            else:
                self.pooler = None
...
@register_model('bert_pair')
class BertHF(FairseqLanguageModel):
    def __init__(self, decoder, task):
        super().__init__(decoder)
        self.task = task

    def forward(self, src_tokens, segment_labels, pairs=None, **unused):
        padding_mask = src_tokens.ne(self.task.dictionary.pad())
        return self.decoder(input_ids=src_tokens, pairs=pairs, token_type_ids=segment_labels, attention_mask=padding_mask)
    ...
    def add_args(parser):
        ...
        parser.add_argument('--clamp-attention', default=False, action='store_true')

    @classmethod
    def build_model(cls, args, task):
        args.no_nsp = getattr(args, 'no_nsp', False)
        args.remove_head = getattr(args, 'remove_head', False)
        args.remove_pooled = getattr(args, 'remove_pooled', False)
        decoder = BertForPreTraining(args.config, args.max_pair_targets, args.pair_positional_embedding_size, args.pair_target_layer, args.remove_head, args.remove_pooled, args.no_nsp)
        return BertHF(decoder, task)

@register_model_architecture('bert_pair', 'bert_pair')
def base_bert_architecture(args):
    ...
    args.config.clamp_attention = getattr(args, 'clamp_attention', args.config.clamp_attention)

@register_model_architecture('bert_pair', 'bert_pair_large')
def large_bert_architecture(args):
    ...
    args.config.clamp_attention = getattr(args, 'clamp_attention', False)
    args.config.initializer_range = getattr(args, 'initializer_range', 0.02)

@register_model_architecture('bert_pair', 'cased_bert_pair')
def cased_base_bert_architecture(args):
    args.config = BertConfig()
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    ```
    """
    def __init__(self, config, max_pair_targets=20, pair_pos_embedding_size=200, pair_target_layer=-1, remove_head = False, remove_pooled=False, no_nsp=False):
        ...
        if not remove_head:
            self.cls = BertPreTrainingHeads(config, max_pair_targets, self.bert.embeddings.word_embeddings.weight, pair_pos_embedding_size, pair_target_layer=pair_target_layer,
                    no_nsp=no_nsp)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, pairs=None, token_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None):
