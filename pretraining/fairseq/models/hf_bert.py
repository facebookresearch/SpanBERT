...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        if self.clamp_attention:
            attention_scores = torch.clamp(attention_scores, -very_small_number, very_small_number)
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, no_nsp):
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * -very_small_number
...
class BertHF(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        decoder = BertForPreTraining(args.config, args.remove_head, args.remove_pooled, args.no_nsp)
...
def base_bert_architecture(args):
    ...
    #args.config.vocab_size = 28996 #args.vocab_size
...
@register_model_architecture('bert_hf', 'cased_bert_hf')
...
@register_model_architecture('bert_hf', 'cased_bert_hf_large')
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def __init__(self, config, remove_head = False, remove_pooled=False, no_nsp=False):
        ...
        self.no_nsp = no_nsp
        if not remove_head:
            self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight, no_nsp)
        self.apply(self.init_bert_weights)
